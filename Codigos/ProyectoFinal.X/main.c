 ...
 * Author: casa
 *
 * Created on 21 de octubre de 2021, 03:24 PM
 */
...
void __interrupt() UART_Rx(void) {//bandera
    ...
    if (RBIF) {//Activo bandera
          RBIF = 0; // la bandera se desactiva 
        if ((PORTB & 0xF0) != 0x00) {//operacion bitwise para activar los flancos
            if (PORTBbits.RB4 == 1) {//Cuando Rb4 sea 1, cumple la condición de adelante  
                __delay_ms(time);
                LATAbits.LA4 = 0;
            }
        } else {
